...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    boolean mEnableLeftNavbar = false; // user defined left navbar
    ...
    private final Runnable mPowerLongPress = new Runnable() {
        ...
        public void run() {
			...
			//******************************************************************
		    //* modify by bonovo zbiao for android box
		    //******************************************************************
			}
	...
	if (DEBUG) Slog.d(TAG, "mLongPressOnHomeBehavior = " + mLongPressOnHomeBehavior);
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onSwipeFromRight() {
                        if (mNavigationBar != null && mEnableRightNavbar) {
                            requestTransientBars(mNavigationBar);
                        }
                    }
                    @Override
                    public void onSwipeFromLeft() {
                        if (mNavigationBar != null && mEnableLeftNavbar) {
                            requestTransientBars(mNavigationBar);
                        }
                    }
                    @Override
    ...
    private void readConfigurationDependentBehaviors() {
        mLongPressOnHomeBehavior = LONG_PRESS_HOME_RECENT_SYSTEM_UI;/*mContext.getResources().getInteger(
	...
	mDensity = density;
        ...
        mNavigationBarCanMove = mEnableRightNavbar||mEnableLeftNavbar;
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            newRightNavBar= Settings.System.getBoolean(resolver,
                    ...
                    false);
            newLeftNavBar= Settings.System.getBoolean(resolver,
                    Settings.System.ENABLE_LEFT_NAVBAR,
                    false);
        ...
        if ((newRightNavBar != mEnableRightNavbar) ||
           (newLeftNavBar != mEnableLeftNavbar)) {
            mEnableRightNavbar = newRightNavBar;
            mEnableLeftNavbar = newLeftNavBar;
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            // while it was pressed, then it is time to go home!
             if (DEBUG) Slog.d(TAG, "^^^^^^^^^^^^repeatCount = " + repeatCount);
            if (!down) {
            ...
            if (repeatCount == 0) {
                mHomePressed = true;
		  mLongPressOnHomeBehavior = LONG_PRESS_HOME_RECENT_SYSTEM_UI ;
	     if (DEBUG) Slog.d(TAG, "myu event.getFlags() is " + event.getFlags() + 
		 		" FLAG_LONG_PRESS is " + KeyEvent.FLAG_LONG_PRESS + " keyguardOn is " + keyguardOn);
                if (mHomeDoubleTapPending) {
                    mHomeDoubleTapPending = false;
        ...
        if (isDefaultDisplay) {
            ...
            if (mNavigationBar != null) {
                ...
                if (mNavigationBarOnBottom) {
                    ...
                    // Landscape screen; nav bar goes to the left or right.
                    if (mEnableLeftNavbar) {
                        int right = mNavigationBarWidthForRotation[displayRotation];
                        mTmpNavigationFrame.set(0, 0, right, displayHeight);
                        mStableLeft = mStableFullscreenLeft = mUnrestrictedScreenLeft = right;
                        if (transientNavBarShowing) {
                            mNavigationBarController.setBarShowingLw(true);
                        } else if (navVisible) {
                            mNavigationBarController.setBarShowingLw(true);
                            mDockLeft = right;
                            mRestrictedScreenLeft = mRestrictedOverscanScreenLeft = right;
                            mRestrictedScreenWidth = mUnrestrictedScreenWidth = mDockRight - mRestrictedScreenLeft;
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        switch (keyCode) {
            ...
            //* add by bonovo zbiao for android box
            //******************************************************************
            case KeyEvent.KEYCODE_BONOVO_UPDATETIME:
				Intent update_intent = new Intent(Intent.ACTION_TIME_CHANGED);
				mContext.sendBroadcastAsUser(update_intent, UserHandle.CURRENT);
				break;
			case KeyEvent.KEYCODE_BONOVO_UPDATEBRIGHTNESS:
				Intent update_brightness_intent = new Intent("android.intent.action.BONOVO_UPDATEBRIGHTNESS_KEY");
				mContext.sendBroadcastAsUser(update_brightness_intent, UserHandle.CURRENT);
				break;
			case KeyEvent.KEYCODE_BONOVO_SHUTDOWN:
				result &= ~ACTION_PASS_TO_USER;
                if (down) {
					mBonovoPowerOff = true;
                    if (isScreenOn && !mPowerKeyTriggered
                            && (event.getFlags() & KeyEvent.FLAG_FALLBACK) == 0) {
                        mPowerKeyTriggered = true;
                        mPowerKeyTime = event.getDownTime();
				...
				break;
			case KeyEvent.KEYCODE_BONOVO_UPDATEVOLUME:
				Intent update_volume_intent = new Intent("android.intent.action.BONOVO_UPDATEVOLUME_KEY");
				mContext.sendBroadcastAsUser(update_volume_intent, UserHandle.CURRENT);
				break;
			case KeyEvent.KEYCODE_BONOVO_UPDATESYSVOL:
				if(down){
					//Log.e(TAG, "+++++++++ KEYCODE_BONOVO_UPDATESYSVOL ++++++++");
					Intent update_sys_volume_intent = new Intent("android.intent.action.KEYCODE_BONOVO_UPDATESYSVOL");
					mContext.sendBroadcast(update_sys_volume_intent);
				}
				break;
			case KeyEvent.KEYCODE_BONOVO_VOLUME_ADD:
				//Log.d(TAG, "============== PhoneWindowManager:BONOVO_VOLUMEADD_KEY down:" + down);
				if(down){
				...
				break;
			case KeyEvent.KEYCODE_BONOVO_VOLUME_SUB:
				//Log.d(TAG, "============== PhoneWindowManager:BONOVO_VOLUMESUB_KEY down:" + down);
				if(down){
				...
				break;
			case KeyEvent.KEYCODE_BONOVO_SYSTEM_MUTE:
				//Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_SYSTEMMUTE down:" + down);
				if(down){
					Intent mute_intent = new Intent("android.intent.action.KEYCODE_BONOVO_SYSTEMMUTE_KEY");
					mContext.sendBroadcastAsUser(mute_intent, UserHandle.CURRENT);
				}
				break;
            case KeyEvent.KEYCODE_BONOVO_SLEEP:
                //Log.d(TAG, "000============== PhoneWindowManager:KEYCODE_BONOVO_SLEEP down:" + down);
                //mPowerManager.goToSleep(SystemClock.uptimeMillis());
                if(down){
                    Intent sleep_intent = new Intent("android.intent.action.BONOVO_SLEEP_KEY");
                    mContext.sendBroadcastAsUser(sleep_intent, UserHandle.CURRENT);
                ...
                break;
            case KeyEvent.KEYCODE_BONOVO_WAKEUP:
                //Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_WAKEUP down:" + down);
                if(down){
                    Intent wakeup_intent = new Intent("android.intent.action.BONOVO_WAKEUP_KEY");
                    mContext.sendBroadcastAsUser(wakeup_intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_RADIO:
                //Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_RADIO");
                if(down){
                    Intent custom_ir_intent = new Intent("android.intent.action.BONOVO_RADIO_KEY");
                    mContext.sendBroadcastAsUser(custom_ir_intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_NAVI:
                //Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_NAVI");
                if(down){
                    Intent naviIntent =  new Intent(Intent.ACTION_MAIN, null);
                    naviIntent.addCategory(Intent.CATEGORY_APP_MAPS);
                    naviIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                    mContext.startActivityAsUser(naviIntent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_CUSTOM_IR_BUTTON:
                //Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_CUSTOM_IR_BUTTON");
                if(down){
                    Intent custom_ir_intent = new Intent("android.intent.action.BONOVO_CUSTOM_IR_BUTTON");
                    mContext.sendBroadcastAsUser(custom_ir_intent, UserHandle.CURRENT);
                }
                break;
            ...
            case KeyEvent.KEYCODE_BONOVO_3G:
                break;
            case KeyEvent.KEYCODE_BONOVO_CAMERA:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_CAMERA");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_BT:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_BT");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_BT_ANSWER:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_BT_ANSWER");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_BT_HANG_UP:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_BT_HANG_UP");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_BT_ANSWER_HANG:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_BT_ANSWER_HANG");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_BT_SWITCH_AUDIO:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_BT_SWITCH_AUDIO");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_RADIO_TURNUP:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_RADIO_TURNUP");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_RADIO_TURNDOWN:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_RADIO_TURNDOWN");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_SWITCH_FMAM:
                if(down){
                    Intent intent = new Intent("android.intent.action.BONOVO_SWITCH_FMAM");
                    mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
                }
                break;
            case KeyEvent.KEYCODE_BONOVO_LIGHTSTATE:
                //Log.d(TAG, "============== PhoneWindowManager:KEYCODE_BONOVO_LIGHTSTATE  down:" + down);
                if(down){
                    Intent lightstate_intent = new Intent("android.intent.action.KEYCODE_BONOVO_LIGHTSTATE");
