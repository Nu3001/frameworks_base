...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    // add by bonovo zbiao for sleep and wakeup
    private final ArrayList<WakeLock> mWakeLocksBackup = new ArrayList<WakeLock>();
    ...
    /*@
     * add by bonovo zbiao for wakeup
     */
    private void acquireWakeLockBeforeWakeup() {
        final int numWakeLocks = mWakeLocksBackup.size();
        for (int i = 0; i < numWakeLocks; i++) {
            WakeLock wakeLock = mWakeLocksBackup.get(i);
            final long ident = Binder.clearCallingIdentity();
            try {
                acquireWakeLockInternal(wakeLock.mLock, wakeLock.mFlags, wakeLock.mTag,
                    wakeLock.mPackageName, wakeLock.mWorkSource, wakeLock.mOwnerUid, wakeLock.mOwnerPid);
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
        mWakeLocksBackup.clear();
    }

    /*@
     * add by bonovo zbiao for sleep
     */
    private void releaseWakeLockBeforeSleep() {
        ...
        for(int i=0; i< mWakeLocksBackup.size(); i++){
            WakeLock wakeLockTemp = mWakeLocksBackup.get(i);
            final long ident = Binder.clearCallingIdentity();
            try {
                releaseWakeLockInternal(wakeLockTemp.mLock, 0);
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
